...
def main() -> None:
    ...
    check_routing(
        save_file=repo_dir / "python-save.md",
        correct_skill="pdb-debugger",
        wrong_skills=["gdb-debugger"],
        case_name="python-case",
        checks=checks,
    )

    check_routing(
        save_file=repo_dir / "native-save.md",
        correct_skill="gdb-debugger",
        wrong_skills=["pdb-debugger"],
        case_name="native-case",
        checks=checks,
    )

    check_routing(
        save_file=repo_dir / "toolchain-save.md",
        correct_skill="cuobjdump-lowering-inspector",
        wrong_skills=["pdb-debugger", "gdb-debugger"],
        case_name="toolchain-case",
        checks=checks,
    )
    ...
    print(json.dumps({
        "passed": True,
        "checks": checks,
        "reason": "all three cases routed to correct skill with no tool spray"
    }, indent=2))
